...
!$$$
 ...
 integer :: i,j, ipts, jpts, iopt, npts, nret, kgds(200)

 real :: fill
 real, allocatable :: rlon(:,:), rlat(:,:), xpts(:,:), ypts(:,:)

! t1534 gaussian
 data kgds/4, 3072, 1536, 89910, 0, 128, -89910, &
           -117, 117, 768, 0, 0,  255, 0, 0, 0, 0, 0, 0, 255, 180*0/
...
! Set lat/lon
 ...
 iopt=-1  ! go from lat/lon to x/y

 call GDSWZD04(KGDS,IOPT,NPTS,FILL,XPTS,YPTS,RLON,RLAT,NRET)

 print*,'x/y: ',xpts(1,1),ypts(1,1)

 deallocate(xpts,ypts,rlon,rlat)

!----------------------------------------------------------------
! To convert all points on the grid.
!----------------------------------------------------------------

! Convert from x/y to lat/lon

 iopt = 1  ! go from x/y to lat/lon

 npts=ipts*jpts

 allocate(rlon(ipts,jpts))
 allocate(rlat(ipts,jpts))
 allocate(xpts(ipts,jpts))
 allocate(ypts(ipts,jpts))

! Set the x/y indices of the points to convert.
! Here, we do the entire grid.
 ...
 print*,'npts/nret ',npts,nret

! The lat/lon of each point is stored in rlat/lon.
! Print out the corner points here.

 print*,'lat 11 ',rlat(1,1),rlon(1,1)
 print*,'lat ul ',rlat(1,jpts),rlon(1,jpts)
 print*,'lat lr ',rlat(ipts,1),rlon(ipts,1)
 print*,'lat ur ',rlat(ipts,jpts),rlon(ipts,jpts)

! Now convert from lat/lon to x/y. Pass in the lat/lon computed in the
! first step.
...
! The x/y indices computed from the lat/lons.
! Print out the corner points here.

 print*,'11 ',xpts(1,1),ypts(1,1)
 print*,'ul ',xpts(1,jpts),ypts(1,jpts)
 print*,'lr ',xpts(ipts,1),ypts(ipts,1)
 print*,'ur ',xpts(ipts,jpts),ypts(ipts,jpts)
